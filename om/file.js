...
const { appSetting, getType } = require('./basic');
...
const read = async function(path=""){
  switch (await isFile(path)){
    case true:
      ...
      } else if(appSetting.data.supportedFileTypes.text){
...
const testFunc = write;

testFunc(testPath, {text: "aaa", array: [1, true, {key: "value"}]}).then((ret) => {
